...
exports.show = async (req, res) => {
  try {
    ...
    let weightedStigmastadiene = 0;

    certificate.items.forEach(item => {
      const itemWeight = parseFloat(item.quantity || 0);
      if (itemWeight > 0) {
        weightedPH += (parseFloat(item.ph || 0) * itemWeight);
        weightedPeroxide += (parseFloat(item.peroxide || item.peroxide_value || 0) * itemWeight);
        weighted232 += (parseFloat(item.abs_232 || item.absorption_232 || 0) * itemWeight);
        weighted266 += (parseFloat(item.abs_266 || item.absorption_266 || 0) * itemWeight);
        weighted270 += (parseFloat(item.abs_270 || item.absorption_270 || 0) * itemWeight);
        weighted274 += (parseFloat(item.abs_274 || item.absorption_274 || 0) * itemWeight);
        weightedDeltaK += (parseFloat(item.delta_k || 0) * itemWeight);
        weightedStigmastadiene += (parseFloat(item.stigmastadiene || 0) * itemWeight);
      }
    });
    ...
    res.render('certificates/show', {
      ...
      total_quantity: certificate.total_quantity,
      totalWeight,
      weightedPH,
      weightedPeroxide,
      weighted232,
